...
RSpec.describe "Retrieves an Asset based on Locator info" do
  ...
  it "finds the asset when multiple locators are present" do
  ...
  it "uses the highest ranked locator to find an asset" do
  ...
  it "ignore higher ranked locators if they do not match any assets" do
  ...
  it "ignores locator keys with blank values" do
  ...
  it "doesn't query with locator keys that have blank values" do
    # without a return option the mock is preventing the code to continue
  ...
  it "adds the http prefix to url locator" do
  ...
  context "no matching assets in db" do
    ...
    # mac -> a
    # h -> b
    it "sets mac address(higher ranked locator) as primary locator" do
      asset = AssetLocator.retrieve(hostname: "some.hostname",
                                    mac_address: "0a:b4:100:99:08:b3")
    ...
    # ip -> a(external)
    # h -> b
    it "creates a new asset and sets external IP(higher ranked locator) as primary locator" do
      start_count = Asset.count
      asset = AssetLocator.retrieve(hostname: "some.hostname",
  ...
  context "there is a matching asset in the db" do
    ...
    it "finds the asset by internal IP, when that is the only locator" do
      asset_in_db = FactoryBot.create(:asset,
    ...
    # h -> a              | h -> a
    # ip -> b(internal)   | ip   -
    it "finds the asset by hostname, internal IP not needed" do
    ...
    it "finds the asset by hostname and updates lower ranked locators" do
      asset_in_db = FactoryBot.create(:asset,
                                       :with_hostname,
      ...
      expect(asset.url_locator).to eq("http://new_test.com") # urls should be scoped to http
    end

    # incoming            | in DB
    # ip -> b(external)   | ip   -
    # h -> a              | h -> a
    it "finds the asset by hostname and improves primary locator" do
      asset_in_db = FactoryBot.create(:asset, :with_hostname)
  ...
  context "asset in db with mismatched higher locator" do
    ...
    # mac -> b            | mac -> c
    # h -> a              | h  -> a
    it "creates a new asset when hostname matches, but mac_address does not" do
      asset_in_db = FactoryBot.create(:asset,
                                       :with_mac_address,
                                       :with_hostname)
      start_count = Asset.count
    ...
    it "creates a new asset when internal IP matches, but hostname does not" do
      asset_in_db = FactoryBot.create(:asset,
                                       :with_internal_ip_address,
                                       :with_hostname)
      start_count = Asset.count
  ...
  it "handles url locator properly when it has _locator" do
